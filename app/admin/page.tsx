...
export default async function AdminPage() {
  ...
  const [unpaid, today, upcoming, completed, cancelled] = await Promise.all([
    ...
    prisma.booking.findMany({
      where: { scheduledAt: { gte: now, lt: weekAhead } },
      orderBy: { scheduledAt: "asc" },
      take: 25,
    }),
    prisma.booking.findMany({
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Section title="Completed (latest 25)" items={completed} />
      <Section title="Cancelled (latest 25)" items={cancelled} />
    </div>
  );
}
...
function Section({ title, items }: { title: string; items: any[] }) {
  return (
    <div style={{ marginBottom: 24 }}>
      ...
      {items.length === 0 ? (
        <div style={{ color: "#6b7280" }}>No records</div>
      ) : (
